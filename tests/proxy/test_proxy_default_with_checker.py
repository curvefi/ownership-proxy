...
def test_default_with_checker_validation(proxy, dummy, mock_checker, dao):
    """Test that __default__ calls the checker when delegation is active"""
    # Set up delegation with a checker
    proxy.set_delegation(
        boa.env.eoa, boa.env.timestamp + 1000, mock_checker.address, sender=dao
    )
...
def test_default_with_checker_revert(proxy, dummy, dao):
...
"""
    ...
    proxy.set_delegation(
        boa.env.eoa, boa.env.timestamp + 1000, reverting_checker.address, sender=dao
    )
...
def test_default_with_conditional_checker(proxy, dummy, dao):
...
"""
    ...
    proxy.set_delegation(
        boa.env.eoa, boa.env.timestamp + 1000, conditional_checker.address, sender=dao
    )
...
def test_default_multiple_delegations_different_checkers(proxy, dummy, dao):
...
"""
    ...
    proxy.set_delegation(user1, boa.env.timestamp + 1000, checker1.address, sender=dao)
    proxy.set_delegation(user2, boa.env.timestamp + 1000, checker2.address, sender=dao)
